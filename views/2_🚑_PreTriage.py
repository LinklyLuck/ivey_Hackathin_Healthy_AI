...
<div class="disclaimer">
    <strong> This is NOT an emergency service.</strong> If you are experiencing a life-threatening emergency,
...
with c_ins:
    search_num = st.text_input("OHIP / UHIP Number:", placeholder="e.g. 4839-227-651-TH", key="pt_search")
with c_email:
    search_email = st.text_input("Email (for verification):", placeholder="e.g. m.thompson@email.com", key="pt_email")
with c_btn:
...
if search_num.strip() and search_email.strip():
    ...
    # Search registered patients: must match BOTH insurance AND email
    if not patients.empty:
        match = pd.DataFrame()
        if "insurance_number" in patients.columns and "email" in patients.columns:
    ...
    if not patient_found:
        st.warning("No patient found. Both insurance number AND email must match for verification.")

elif search_num.strip() or search_email.strip():
    st.info("Please enter **both** your insurance number and email for identity verification.")
...
if chief_complaint and st.session_state.triage_step == "input":
    if st.button(" Analyze Symptoms & Get Questions", type="primary", use_container_width=True):
...
if st.session_state.triage_step in ("questions", "result") and st.session_state.triage_questions:
    ...
    if ai_data.get("red_flags_detected"):
        st.markdown(f'<div class="safety-banner"><strong> Red Flags:</strong> {", ".join(ai_data["red_flags_detected"])}</div>', unsafe_allow_html=True)
    ...
    symptom_keys = {
        ...
        "none_of_above": "N/A — None of the above",
    }
    answers = {}
    c1, c2 = st.columns(2)
    ...
    if calc:
        ...
        st.markdown(f"""
        <div style="padding:1rem;margin:0.5rem 0;background:#FFFBEB;border:2px solid #F59E0B;border-radius:8px;text-align:center;">
            <p style="font-size:1.1rem;color:#92400E;margin:0;"> Please remember your Case Number:</p>
            <p style="font-size:2rem;font-weight:800;color:#DC2626;margin:0.3rem 0;">{case_id}</p>
            <p style="font-size:0.9rem;color:#92400E;margin:0;">You will need this for Follow-Up and to check your results.</p>
        </div>
        """, unsafe_allow_html=True)
